...
**IMPORTANT: This application only accepts PDF documents. All document processing features described below apply exclusively to PDF files.**
...
## 1. Multi-Modal PDF Document Processing

Enhance PDF document processing to handle visual elements alongside text extraction:
...
class MultiModalProcessor:
    ...
    def process_pdf_document(self, pdf_path: str, user_selected_category: str):
        # Extract text, tables, charts, and images from PDF
        text_content = self.text_extractor.extract(pdf_path)
        visual_elements = self.image_processor.extract_visual_elements(pdf_path)
        tables = self.table_extractor.extract_tables(pdf_path)
        layout = self.layout_analyzer.analyze_structure(pdf_path)
        
        # Process based on user-selected category with visual context
        return self.category_specific_processing(
            text_content, visual_elements, tables, layout, user_selected_category
        )
...
class HybridRetriever:
    def __init__(self):
        self.chroma_cloud = ChromaCloudClient()
    ...
    def retrieve(self, query: str, category: str, top_k: int = 10) -> List[Document]:
        ...
        semantic_results = self.chroma_cloud.similarity_search(
